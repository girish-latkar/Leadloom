        ...
        <div className="mt-16">
          <Reveal>
            <div className="font-mono text-xs tracking-[0.14em] text-grey uppercase">
              How the match gets made
            </div>
            <h3 className="mt-3 max-w-[520px] font-display text-[clamp(24px,3vw,32px)] leading-[1.12] font-medium tracking-[-0.02em]">
              Three checks happen before anyone sees a lead.
            </h3>
          </Reveal>

          <div className="mt-10 grid grid-cols-3 gap-px border border-line bg-line max-[820px]:grid-cols-1">
            {PROCESS_STEPS.map((step, index) => (
              <Reveal
                key={step.num}
                delay={index * 0.08}
                className="bg-ink-card px-7 py-7 transition-[background-color,transform] duration-[350ms] ease-out-loom hover:-translate-y-1 hover:bg-ink-soft max-sm:px-5 max-sm:py-6"
              >
                <div className="font-mono text-[13px] text-gold">{step.num}</div>
                <h4 className="mt-4 font-display text-[19px] font-medium tracking-[-0.01em]">
                  {step.title}
                </h4>
