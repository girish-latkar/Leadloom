...
interface GetStartedModalBodyProps {
  initialAudience?: Audience;
  defaultAudience: Audience;
  onClose: () => void;
}

function GetStartedModalBody({ initialAudience, defaultAudience, onClose }: GetStartedModalBodyProps) {
  const [formAudience, setFormAudience] = useState<Audience>(initialAudience ?? defaultAudience);
...
export function GetStartedModal({
  ...
  return (
    <Modal open={open} onClose={onClose} label="Find My Designer" variant="sheet" showCloseButton={false}>
      {open ? (
        <GetStartedModalBody
          key={`${initialAudience ?? audience}`}
